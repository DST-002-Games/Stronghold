...
public class DailyTimerTask implements Runnable {
    ...
    public DailyTimerTask(Stronghold plugin) {
        this.plugin = plugin;
            ...
            if (econ != null) {
                ...
                if (newBalance < 0 && plugin.getConfigManager().getDestroyNoMoney()) {
                    ...
                    new Runnable() {
                          @Override
                          public void run() {
                                plugin.getServer().broadcastMessage(ChatColor.RED + "[Stronghold] " + st + " ran out of money!");
                          }
                    }.run();
                } else {
            ...
            if (cm.getUsePower()) {
                ...
                if (power <= 0 && plugin.getConfigManager().getDestroyNoPower()) {
                    ...
                    new Runnable() {
                          @Override
                          public void run() {
                                plugin.getServer().broadcastMessage(ChatColor.RED + "[Stronghold] " + st + " has no power!");
                          }
                    }.run();
