...
public class Effect
{
	...
	public Effect(Stronghold plugin)
	{
		this.plugin = plugin;
	}
	...
	public Region getContainingRegion(Location currentLocation)
	{
		...
		for (Region r : rm.getSortedRegions())
		{
			...
			{
				if (!(l.getX() - radius > x1)
						&& l.distanceSquared(currentLocation) < radius)
				{
					loc = l;
					break;
				}
	 ...
	 * to Stronghold, only you decide what that number means. Example:
	 * arrowturrets use the number to determine arrow velocity. Conveyors use
	 * the number as the item id that needs to be moved.
	 * 
	 * @param effects
	 *            regionManager.getRegionType(region.getType()).getEffects()
	 * @param name
	 *            example: "denyblockbreak"
	 * @return the number following the effect (returns 0 if doesn't have that
	 *         effect)
	 */
	...
	{
		int data = 0;
		ArrayList<String> effects = plugin.getRegionManager()
	 ...
	 * @param player
	 * @param location
	 *            the location of the region center region.getLocation()
	 * @return
	 */
	public boolean isOwnerOfRegion(Player p, Location l)
			throws NullPointerException
	{
		return getPlugin().getRegionManager().getRegion(l).getOwners()
				.contains(p.getName());
	}
	 ...
	 * 
	 * @exception thows
	 *                NPE if no region found at the given location
	 * @param player
	 * @param location
	 *            the location of the region center region.getLocation()
	 * @return
		...
		} else
		{
			for (String s : r.getMembers())
			{
				if (s.contains("sr:"))
				{
					...
					if (sr != null
							&& (sr.hasMember(p.getName()) || sr.hasOwner(p
									.getName())))
					{
						return true;
					}
	 ...
	 * 
	 * @param location
	 *            the location of the center of the region region.getLocation()
	 * @return
	 */
	public boolean hasReagents(Location l)
	{
		...
		/*
		 * Map<Material, Integer> reagentMap = new EnumMap<Material,
		 * Integer>(Material.class); for (ItemStack is : rt.getReagents()) {
		...
		{
			ArrayList<SuperRegion> srs = rm.getContainingSuperRegions(r
					.getLocation());
			if (srs.isEmpty())
			{
				return false;
			...
			{
				if (sr.getPower() > rt.getPowerDrain()
						&& rm.getSuperRegionType(sr.getType()).getMaxPower() > sr
								.getPower() - rt.getPowerDrain())
				{
					hasPower = true;
					break;
				}
		...
		if ((bs instanceof Chest) == false)
		{
			System.out.println("HS no Chest id:" + r.getID());
		}
		...
		return Util.containsItems(rt.getReagents(), inventory);

		/*
		 * for (ItemStack is : rt.getReagents()) { if
		 * (!chest.getInventory().contains(is.getType(), is.getAmount())) {
		 * return false; } } return true;
		 */
		/*
		 * for (ItemStack is : chest.getInventory().getContents()) { Material
		 * mat = Material.AIR; if (is != null) { mat = is.getType(); if
		 * (!mat.equals(Material.AIR) && reagentMap.containsKey(mat)) { if
		 * (reagentMap.get(mat) <= is.getAmount()) { reagentMap.remove(mat); }
		 * else { reagentMap.put(mat, reagentMap.get(mat) - is.getAmount()); } }
		 * } } if (reagentMap.isEmpty()) { return true; }
		 * 
		 * return false;
		 */
	}
	...
	public void forceUpkeep(Location l)
	{
		...
		// Remove the upkeep items from the region chest and add items from
		// output
		...
		/*
		 * boolean hasUpkeep = true; for (ItemStack is : rt.getUpkeep()) { if
		 * (!chest.getInventory().contains(is.getType(), is.getAmount())) {
		 * hasUpkeep = false; } } if (!hasUpkeep) { return; }
		 */
		...
		if (rt.getMoneyOutput() != 0 && Stronghold.econ != null)
		{
			if (output < 0
					&& Stronghold.econ.getBalance(playername) < Math
							.abs(output))
			{
				return;
			}
		...
		if (Stronghold.econ != null)
		{
			if (output < 0)
			{
				Stronghold.econ.withdrawPlayer(playername, Math.abs(output));
			} else
			{
				Stronghold.econ.depositPlayer(playername, output);
			}
		}
	...
	/**
	 * Forces the region to consume upkeep items and output money and items if a
	 * random number is lower than the upkeep-chance.
	 * 
	...
	public boolean upkeep(Location l)
	{
		 ...
		 * boolean hasUpkeep = true; for (ItemStack is : rt.getUpkeep()) { if
		 * (!chest.getInventory().contains(is.getType(), is.getAmount())) {
		 * hasUpkeep = false; } } if (!hasUpkeep) { return false; }
		 */
		...
		double output = rt.getMoneyOutput();
		if (output != 0 && Stronghold.econ != null)
		{
			Economy econ = Stronghold.econ;
