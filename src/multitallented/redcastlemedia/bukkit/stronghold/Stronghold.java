...
import multitallented.redcastlemedia.bukkit.stronghold.checkregiontask.CheckRegionTask;
import multitallented.redcastlemedia.bukkit.stronghold.effect.EffectManager;
import multitallented.redcastlemedia.bukkit.stronghold.events.CommandEffectEvent;
import multitallented.redcastlemedia.bukkit.stronghold.listeners.*;
import multitallented.redcastlemedia.bukkit.stronghold.region.*;
...
public class Stronghold extends JavaPlugin
{
	...
	public void onEnable()
	{
		...
		// //Check for Heroes
		// log.info("[SH] is looking for Heroes...");
		// Plugin currentPlugin = pm.getPlugin("Heroes");
		// if (currentPlugin != null) {
		// log.info("[SH] found Heroes!");
		// heroes = ((Heroes) currentPlugin);
		// } else {
		// log.info("[SH] didnt find Heroes, waiting for Heroes to be enabled.");
		// }
		...
		getServer().getScheduler().scheduleSyncRepeatingTask(this, theSender,
				10L, 10L);
		...
		date.setMinutes(0);
		date.setHours(0);
		long timeUntilDay = (86400000 + date.getTime() - System
				.currentTimeMillis()) / 50;
		System.out.println("[SH] " + timeUntilDay + " ticks until 00:00");
		DailyTimerTask dtt = new DailyTimerTask(this);
		getServer().getScheduler().scheduleSyncRepeatingTask(this, dtt,
				timeUntilDay, 1728000);

		log.info("[SH] is now enabled!");
	}
...
//		{
//			if (player != null
//					&& !(Stronghold.perms == null || Stronghold.perms.has(
//							player, "stronghold.admin")))
//			{
//				return true;
//			}
//			config = getConfig();
...
//			sender.sendMessage("[SH] reloaded");
//			return true;
//		}
		if (player == null)
		{
			sender.sendMessage("[SH] doesn't recognize non-player commands.");
			return true;
		}
		// WAR
		...
		// "[SH] That isn't a valid super-region.");
		// return true;
		// }
		//
		// //Check if already at war
		// if (regionManager.hasWar(sr1, sr2)) {
		// player.sendMessage(ChatColor.GRAY + "[SH] " + sr1.getName() +
		// " is already at war!");
		// return true;
		// }
		//
		...
		// player.sendMessage(ChatColor.GRAY + "[SH] You are not an owner of " +
		// sr1.getName());
		// return true;
		// }
		//
		...
		// if (!cm.getUseWar()) {
		// player.sendMessage(ChatColor.GRAY +
		// "[SH] This command is disabled in config.yml");
		// return true;
		// }
		// double cost = cm.getDeclareWarBase() + cm.getDeclareWarPer() *
		// (sr1.getOwners().size() + sr1.getMembers().size() +
		...
		// player.sendMessage(ChatColor.GRAY + "[SH] " + sr1.getName() +
		// " doesn't have enough money to war with " + sr2.getName());
		// return true;
		// } else {
		...
		// public void run()
		// {
		// getServer().broadcastMessage(ChatColor.RED + "[SH] " + sr1a.getName()
		// + " has declared war on " + sr2a.getName() + "!");
		// }
		// }.run();
		// return true;
		// } else
		// if (args.length > 2 && args[0].equalsIgnoreCase("peace"))
		// {
		// //hs peace mySR urSR
		//
		// //Check for valid super-regions
		...
		// return true;
		// }
		// double cost = cm.getMakePeaceBase() + cm.getMakePeacePer() *
		// (sr1.getOwners().size() + sr1.getMembers().size() +
		// sr2.getOwners().size() + sr2.getMembers().size());
		//
		// //Check money
		...
		// if (sr1.getBalance() < cost) {
		// player.sendMessage(ChatColor.GRAY + "[SH] " + sr1.getName() +
		// " doesn't have enough money to make peace with " + sr2.getName());
		// return true;
		// } else {
		// regionManager.addBalance(sr1, -1 * cost);
		// }
		...
		// {
		// getServer().broadcastMessage(ChatColor.RED + "[SH] " + sr1a.getName()
		// + " has made peace with " + sr2a.getName() + "!");
		// }
		// }.run();
		// return true;
		// } else
		if (args.length > 2 && args[0].equalsIgnoreCase("charter"))
		{
			...
			SuperRegionType currentRegionType = regionManager
					.getSuperRegionType(args[1]);
			if (currentRegionType == null)
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + args[1]
						+ " isnt a valid region type");
				int j = 0;
				String message = ChatColor.GOLD + "";
				for (String s : regionManager.getSuperRegionTypes())
				{
					if (perms == null
							|| (perms.has(player, "herostronghold.create.all") || perms
									.has(player, "herostronghold.create." + s)))
					{
				...
				{
					player.sendMessage(message.substring(0,
							message.length() - 2));
				}
				return true;
			}
			...
			// Permission Check
			if (perms != null
					&& !perms.has(player, "herostronghold.create.all")
					&& !perms.has(player, "herostronghold.create."
							+ regionTypeName))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] you dont have permission to create a "
						+ regionTypeName);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + args[1]
						+ " doesnt require a charter. /hs create " + args[1]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Sorry but that name is too long. (16 max)");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Sorry but that is an invalid filename.");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is already a charter or region with that name.");
				return true;
			}
			if (getServer().getPlayerExact(args[2]) != null)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Dont name a super-region after a player");
				return true;
			}

			// Check if allowed super-region
			if (regionManager.getSuperRegion(args[2]) != null
					&& (!regionManager.getSuperRegion(args[2]).hasOwner(
							player.getName()) || regionManager
							.getSuperRegion(args[2]).getType()
							.equalsIgnoreCase(args[1])))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] That exact super-region already exists.");
				return true;
			}
			...
			player.sendMessage(ChatColor.GOLD
					+ "[SH] Youve successfully created a charter for "
					+ args[2]);
			player.sendMessage(ChatColor.GOLD
					+ "[SH] Get other people to type /hs signcharter "
					+ args[2] + " to get started.");
			return true;
		} else if (args.length > 1 && args[0].equalsIgnoreCase("charterstats"))
		{
			...
			} else
			{
				player.sendMessage(ChatColor.RED
						+ "[SH] There was an error loading that charter");
		...
		} else if (args.length > 1 && args[0].equalsIgnoreCase("signcharter"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[Herostronghold] There is no charter for " + args[1]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You dont have permission to sign a charter.");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You've already signed this charter.");
				return true;
			}
			...
			player.sendMessage(ChatColor.GOLD
					+ "[SH] You just signed the charter for " + args[1]);
			int remaining = 0;
			SuperRegionType srt = regionManager.getSuperRegionType(charter
					.get(0));
			if (srt != null)
			{
			...
			if (remaining > 0)
			{
				player.sendMessage(ChatColor.GOLD + "" + remaining
						+ " signatures to go!");
			}
			Player owner = getServer().getPlayer(charter.get(1));
			if (owner != null && owner.isOnline())
			{
				owner.sendMessage(ChatColor.GOLD + "[SH] "
						+ player.getDisplayName()
						+ " just signed your charter for " + args[1]);
				if (remaining > 0)
				{
					owner.sendMessage(ChatColor.GOLD + "" + remaining
							+ " signatures to go!");
				}
			}
			return true;
		} else if (args.length > 1 && args[0].equals("cancelcharter"))
		{
			...
			if (pendingCharters.get(args[1]).size() < 2
					|| !pendingCharters.get(args[1]).get(1)
							.equals(player.getName()))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are the not owner of this charter.");
				return true;
			}
			...
			player.sendMessage(ChatColor.GOLD
					+ "[SH] You have canceled the charter for " + args[1]);
			return true;
		} else
		// CREATE REGION
		if (args.length == 2 && args[0].equalsIgnoreCase("create"))
		{
			...
			boolean createAll = nullPerms
					|| perms.has(player, "herostronghold.create.all");
			if (!(nullPerms || createAll || perms.has(player,
			...
			// Check if player is standing someplace where a chest can be
			// placed.
			Block currentBlock = currentLocation.getBlock();
			if (currentBlock.getTypeId() != 0)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] please stand someplace where a chest can be placed.");
				return true;
			}
			RegionType currentRegionType = regionManager
					.getRegionType(regionName);
			if (currentRegionType == null)
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + regionName
						+ " isnt a valid region type");
				player.sendMessage(ChatColor.GRAY + "[SH] Try /hs create "
						+ regionName + " <insert_name_here>");
				return true;
			}
			...
			if (econ != null)
			{
				...
				{
					player.sendMessage(ChatColor.GRAY + "[SH] You need $"
							+ cost + " to make this type of structure.");
					return true;
				} else
				{
					costCheck = cost;
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You dont have permission to build more "
						+ currentRegionType.getName());
				return true;
			}

			// Check biome
			if (!currentRegionType.getBiome().isEmpty()
					&& !currentRegionType.getBiome().contains(
							player.getLocation().getBlock().getBiome().name()))
			{
				...
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You must build this in a " + mes + " biome");
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are currently in a "
						+ player.getLocation().getBlock().getBiome().name()
						+ " biome");
				return true;
			}
			...
			if (!regionManager.getContainingBuildRegions(currentLocation)
					.isEmpty())
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are too close to another Stronghold");
				return true;
			}

			// Check if in a super region and if has permission to make that
			// region
			String playername = player.getName();
			List<String> reqSuperRegion = currentRegionType.getSuperRegions();
			boolean meetsReqs = reqSuperRegion == null
					|| reqSuperRegion.isEmpty();
			for (SuperRegion sr : regionManager
					.getContainingSuperRegions(currentLocation))
			{
				if (!meetsReqs && reqSuperRegion != null
						&& reqSuperRegion.contains(sr.getType()))
				{
					meetsReqs = true;
				}
				if (!sr.hasOwner(playername))
				{
					if (!sr.hasMember(playername)
							|| !sr.getMember(playername).contains(regionName))
					{
						player.sendMessage(ChatColor.GRAY
								+ "[SH] You dont have permission from an owner of "
								+ sr.getName() + " to create a " + regionName
								+ " here");
						return true;
					}
				}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are required to build this " + regionName
						+ " in a:");
				String message = ChatColor.GOLD + "";
				int j = 0;
			...
			ArrayList<ItemStack> requirements = currentRegionType
					.getRequirements();
			...
			if (!requirements.isEmpty())
			{
				...
				{
					reqMap.put(new Integer(currentIS.getTypeId()), new Integer(
							currentIS.getAmount()));
				}

				// Check the area for required blocks
				int radius = (int) Math
						.sqrt(currentRegionType.getBuildRadius());
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] you don't have all of the required blocks in this structure.");
				String message = ChatColor.GOLD + "";
				int j = 0;
			...
			// Create chest at players feet for tracking reagents and removing
			// upkeep items
			...
			// if (heroes != null) {
			// heroes.getCharacterManager().getHero(player).gainExp(currentRegionType.getExp(),
			// ExperienceType.EXTERNAL, player.getLocation());
			// }
			regionManager.addRegion(currentLocation, regionName, owners);
			player.sendMessage(ChatColor.GRAY + "[SH] " + ChatColor.WHITE
					+ "You successfully create a " + ChatColor.RED + regionName);
			...
			if (currentRegionType.getReagents() == null
					|| currentRegionType.getReagents().isEmpty())
			{
				message += "None";
				player.sendMessage(message);
			} else
		...
		if (args.length > 2 && args[0].equalsIgnoreCase("create"))
		{
			...
			Location currentLocation = player.getLocation();
			SuperRegionType currentRegionType = regionManager
					.getSuperRegionType(regionTypeName);
			if (currentRegionType == null)
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + regionTypeName
						+ " isnt a valid region type");
				int j = 0;
				String message = ChatColor.GOLD + "";
				for (String s : regionManager.getSuperRegionTypes())
			...
			if (econ != null)
			{
				...
				{
					player.sendMessage(ChatColor.GRAY + "[SH] You need $"
							+ cost + " to make this type of region.");
					return true;
				} else
				{
					costCheck = cost;
			...
			if (!Stronghold.perms.has(player, "herostronghold.admin"))
			{
				if (currentCharter > 0)
				{
					try
					{
						...
						{
							player.sendMessage(ChatColor.GRAY
									+ "[SH] You need to start a charter first. /hs charter "
									+ args[1] + " " + args[2]);
							return true;
						} else if (pendingCharters.get(args[2]).size() <= currentCharter)
						{
							player.sendMessage(ChatColor.GRAY
									+ "[SH] You need " + currentCharter
									+ " signature(s). /hs signcharter "
									+ args[2]);
							return true;
						} else if (!pendingCharters.get(args[2]).get(0)
								.equalsIgnoreCase(args[1])
								|| !pendingCharters.get(args[2]).get(1)
										.equalsIgnoreCase(player.getName()))
						{
							player.sendMessage(ChatColor.GRAY
									+ "[SH] The charter for this name is for a different region type or owner.");
							player.sendMessage(ChatColor.GRAY + "Owner: "
									+ pendingCharters.get(args[2]).get(1)
									+ ", Type: "
									+ pendingCharters.get(args[2]).get(0));
							return true;
						} else
						{
							int i = 0;
					...
					{
						warning("Possible failure to find correct charter for "
								+ args[2]);
					}
				}
			...
			Map<String, Integer> requirements = currentRegionType
					.getRequirements();
			...
			// Check if there already is a super-region by that name, but not if
			// it's one of the child regions
			if (regionManager.getSuperRegion(args[2]) != null
					&& (children == null || !children.contains(regionManager
							.getSuperRegion(args[2]).getType())))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is already a super-region by that name.");
				return true;
			}
			...
			for (SuperRegion sr : regionManager.getSortedSuperRegions())
			{
				try
				{
					if (sr.getLocation().distance(currentLocation) < radius
							+ regionManager.getSuperRegionType(sr.getType())
									.getRawRadius()
							&& (sr.getType().equalsIgnoreCase(regionTypeName) || !sr
									.hasOwner(player.getName())))
					{
						player.sendMessage(ChatColor.GRAY + "[SH] "
								+ sr.getName() + " is already here.");
						return true;
					}
			...
			if (!req.isEmpty())
			{
						...
						.getContainingSuperRegions(currentLocation))
				{
					...
					if (!sr.hasOwner(player.getName())
							&& (!sr.hasMember(player.getName()) || !sr
									.getMember(player.getName()).contains(
											regionTypeName)))
					{
						player.sendMessage(ChatColor.GRAY
								+ "[SH] You are not permitted to build a "
								+ regionTypeName + " inside " + sr.getName());
						return true;
					}
				...
				for (Region r : regionManager.getSortedRegions())
				{
					...
					if (l.getX() - radius1 < x && l.getY() + radius1 > y
							&& l.getY() - radius1 < y && l.getZ() + radius1 > z
							&& l.getZ() - radius1 < z
				...
				if (!req.isEmpty())
				{
					for (Region r : regionManager
							.getContainingRegions(currentLocation))
					{
						String rType = regionManager.getRegion(r.getLocation())
								.getType();
						if (req.containsKey(rType))
						{
							int amount = req.get(rType);
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] This area doesnt have all of the required regions.");
				int j = 0;
				String message = ChatColor.GOLD + "";
				for (String s : req.keySet())
				{
					if (message.length() + s.length() + 3
							+ req.get(s).toString().length() > 55)
					{
						player.sendMessage(message);
						message = ChatColor.GOLD + "";
						j++;
			...
			for (String s : quietDestroy)
			{
				...
				for (String sm : sr.getMembers().keySet())
				{
					if (!members.containsKey(sm)
							&& sr.getMember(sm).contains("member"))
						members.put(sm, sr.getMember(sm));
				}
				balance += sr.getBalance();
			}
			// Check if more members needed to create the super-region
			if (owners.size() + members.size() < currentRegionType
					.getPopulation())
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You need "
						+ (currentRegionType.getPopulation() - owners.size() - members
								.size()) + " more members.");
				return true;
			}
			for (String s : quietDestroy)
			...
			// if (hero.hasParty()) {
			// hero.getParty().gainExp(currentRegionType.getExp(),
			// ExperienceType.EXTERNAL, player.getLocation());
			// } else {
			// heroes.getCharacterManager().getHero(player).gainExp(currentRegionType.getExp(),
			// ExperienceType.EXTERNAL, player.getLocation());
			// }
			// }
			regionManager.addSuperRegion(args[2], currentLocation,
					regionTypeName, owners, members,
					currentRegionType.getDailyPower(), balance);
			player.sendMessage(ChatColor.GOLD + "[SH] You've created a new "
					+ args[1] + " called " + args[2]);
			return true;
		} else if (args.length > 0 && args[0].equalsIgnoreCase("listall"))
		{
			if (args.length > 1)
			{
				...
				{
					player.sendMessage(ChatColor.GRAY
							+ "[SH] There is no super-region type named "
							+ args[1]);
					return true;
				}
				String message = ChatColor.GOLD + "";
		...
		} else if (args.length > 2 && args[0].equalsIgnoreCase("withdraw"))
		{
			...
			try
			{
				...
				{
					player.sendMessage(ChatColor.GRAY
							+ "[SH] Withdraw a positive amount only.");
					return true;
				}
			} catch (Exception e)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Invalid amount /hs withdraw <amount> <superregionname>");
				return true;
			}
			...
			if (sr == null)
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + args[2]
						+ " is not a super-region");
				return true;
			}

			// Check if owner or permitted member
			if ((!sr.hasMember(player.getName()) || !sr.getMember(
					player.getName()).contains("withdraw"))
					&& !sr.hasOwner(player.getName()))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are not a member or dont have permission to withdraw");
				return true;
			}

			// Check if bank has that money
			double output = regionManager.getSuperRegionType(sr.getType())
					.getOutput();
			if (output < 0 && sr.getBalance() - amount < -output)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You cant withdraw below the the minimum required.");
				return true;
			} else if (output >= 0 && sr.getBalance() - amount < 0)
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + sr.getName()
						+ " doesnt have that much money.");
				return true;
			}
			...
			player.sendMessage(ChatColor.GOLD + "[SH] You withdrew " + amount
					+ " in the bank of " + args[2]);
			return true;
		} else if (args.length > 2 && args[0].equalsIgnoreCase("deposit"))
		{
			...
			try
			{
				...
				{
					player.sendMessage(ChatColor.GRAY
							+ "[SH] Nice try. Deposit a positive amount.");
					return true;
				}
			} catch (Exception e)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Invalid amount /hs deposit <amount> <superregionname>");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You dont have that much money.");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are not a member of " + args[2]);
				return true;
			}
			...
			regionManager.addBalance(sr, amount);
			player.sendMessage(ChatColor.GOLD + "[SH] You deposited " + amount
					+ " in the bank of " + args[2]);
			return true;
		} else if (args.length > 2 && args[0].equalsIgnoreCase("settaxes"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no region called " + args[2]);
				return true;
			}
			if (!sr.hasOwner(playername) && !sr.hasMember(playername))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You dont have permission to set taxes for "
						+ args[2] + ".");
				return true;
			}
			...
			if (sr.hasMember(playername)
					&& !sr.getMember(playername).contains("settaxes"))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You dont have permission to set taxes for "
						+ args[2] + ".");
			...
			try
			{
				...
				{
					player.sendMessage(ChatColor.GRAY
							+ "[SH] You cant set taxes that high/low.");
					return true;
				}
			} catch (Exception e)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Use /hs settaxes <amount> <superregionname>.");
				return true;
			}

			// Set the taxes
			regionManager.setTaxes(sr, taxes);
			player.sendMessage(ChatColor.GOLD + "[SH] You've set " + args[2]
					+ "'s taxes to " + args[1]);
			return true;
		} else if (args.length > 2 && args[0].equalsIgnoreCase("listperms"))
		{
			...
			if (args.length > 3)
			{
				...
				{
					player.sendMessage(ChatColor.GOLD + "[SH] Could not find "
							+ args[1]);
					return true;
				} else
				{
			...
			String message = ChatColor.GRAY + "[SH] " + playername
					+ " perms for " + args[2] + ":";
			...
			} else if (sr.hasMember(playername))
			{
				...
				{
					player.sendMessage(message2.substring(0,
							message2.length() - 2));
				}
				return true;
			}
			player.sendMessage(ChatColor.GRAY + "[SH] " + playername
					+ " doesn't belong to that region.");
			return true;
		} else if (args.length > 0 && args[0].equalsIgnoreCase("listallperms"))
		{
			player.sendMessage(ChatColor.GRAY
					+ "[SH] List of all member permissions:");
			player.sendMessage(ChatColor.GRAY
					+ "member = is a member of the super-region.");
			player.sendMessage(ChatColor.GRAY
					+ "title:<title> = player's title in channel");
			player.sendMessage(ChatColor.GRAY
					+ "addmember = lets the player use /hs addmember");
			player.sendMessage(ChatColor.GRAY
					+ "<regiontype> = lets the player build that region type");
			player.sendMessage(ChatColor.GRAY
					+ "withdraw = lets the player withdraw from the bank");
			return true;
		} else if (args.length > 0 && args[0].equalsIgnoreCase("ch"))
		{
			// Check if wanting to be set to any other channel
			if (args.length == 1 || args[1].equalsIgnoreCase("o")
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] /hs ch channelname.  /hs ch (to go to all chat)");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no super-region by that name ("
						+ args[1] + ").");
				player.sendMessage(ChatColor.GRAY
						+ "Try /hs ch to go to all chat.");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You must be a member of " + args[1]
						+ " before joining thier channel");
				return true;
			}
		...
		} else if (args.length > 2 && args[0].equalsIgnoreCase("addmember"))
		{
			...
			boolean isAdmin = Stronghold.perms.has(player,
					"herostronghold.admin");
			if (!isMember && !isOwner && !isAdmin)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You arent a member of " + args[2]);
				return true;
			}

			// Check if player has permission to invite players
			if (!isAdmin && isMember
					&& !sr.getMember(playername).contains("addmember"))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You need permission addmember from an owner of "
						+ args[2]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + args[1]
						+ " doesnt have permission to join a super-region.");
				return true;
			}

			// Check if has housing effect and if has enough housing
			if (regionManager.getSuperRegionType(sr.getType()).hasEffect(
					"housing")
					&& !regionManager.hasAvailableHousing(sr))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You cant addmember people to " + sr.getName()
						+ " until you build more housing");
				return true;
			}

			// Send an invite
			pendingInvites.put(invitee.getName(), args[2]);
			player.sendMessage(ChatColor.GRAY + "[SH] You have invited "
					+ ChatColor.GOLD + invitee.getDisplayName()
					+ ChatColor.GRAY + " to join " + ChatColor.GOLD + args[2]);
			if (invitee != null)
				invitee.sendMessage(ChatColor.GOLD
						+ "[SH] You have been invited to join " + args[2]
						+ ". /hs accept " + args[2]);
			return true;
		} else if (args.length > 1 && args[0].equalsIgnoreCase("accept"))
		{
			// Check if player has a pending invite to that super-region
			if (!pendingInvites.containsKey(player.getName())
					|| !pendingInvites.get(player.getName()).equals(args[1]))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You don't have an invite to " + args[1]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are already a member of " + args[1]);
				return true;
			}
			...
			player.sendMessage(ChatColor.GOLD + "[SH] Welcome to " + args[1]);
			for (String s : sr.getMembers().keySet())
			{
				Player p = getServer().getPlayer(s);
				if (p != null)
				{
					p.sendMessage(ChatColor.GOLD + playername + " has joined "
							+ args[1]);
				}
			}
		...
		} else if (args.length > 2 && args[0].equalsIgnoreCase("addowner"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no super-region named " + args[2]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[Herostronghold] There is no player online named: "
						+ args[1]);
				return true;
			} else
			{
			...
			if (!sr.hasOwner(player.getName())
					&& !Stronghold.perms.has(player, "herostronghold.admin"))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You arent an owner of " + args[2]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + args[1]
						+ " is not a member of " + args[2]);
				return true;
			}

			regionManager.removeMember(sr, playername);
			if (p != null)
				p.sendMessage(ChatColor.GOLD + "[SH] You are now an owner of "
						+ args[2]);
			for (String s : sr.getMembers().keySet())
			{
				...
				if (pl != null)
				{
					pl.sendMessage(ChatColor.GOLD + playername
							+ " is now an owner of " + args[2]);
				}
			}
			for (String s : sr.getOwners())
			{
		...
		} else if (args.length > 2 && args[0].equalsIgnoreCase("remove"))
		{
			...
			if (playername.equalsIgnoreCase(player.getName()))
			{
				...
				player.sendMessage(ChatColor.GRAY + "[SH] You have left "
						+ args[2]);
				for (String s : sr.getMembers().keySet())
				{
					Player pl = getServer().getPlayer(s);
					if (pl != null)
					{
						pl.sendMessage(ChatColor.GOLD + playername + " left "
								+ args[2]);
					}
				}
				for (String s : sr.getOwners())
			...
			if (!sr.hasOwner(player.getName())
					&& !(!sr.hasMember(player.getName()) || !sr.getMember(
							player.getName()).contains("remove")) && !isAdmin)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You don't have permission to remove that member.");
				return true;
			}
			...
			if (p != null)
				p.sendMessage(ChatColor.GRAY
						+ "[SH] You are no longer a member of " + args[2]);
		...
		} else if (args.length > 3 && args[0].equalsIgnoreCase("toggleperm"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no super-region named " + args[3]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You aren't an owner of " + args[3]);
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[Herostronghold] There is no player named: "
						+ args[1]);
				return true;
			} else if (p != null)
			{
			...
			{
				player.sendMessage(ChatColor.GRAY + "[SH] " + args[1]
						+ " either owns, or is not a member of " + args[3]);
				return true;
			}
			...
			if (perm.contains(args[2]))
			{
				...
				player.sendMessage(ChatColor.GRAY + "[SH] Removed perm "
						+ args[2] + " for " + args[1] + " in " + args[3]);
				if (p != null)
					p.sendMessage(ChatColor.GRAY + "[SH] Your perm " + args[2]
							+ " was revoked in " + args[3]);
				return true;
			} else
			{
				perm.add(args[2]);
				regionManager.setMember(sr, playername, perm);
				player.sendMessage(ChatColor.GRAY + "[SH] Added perm "
						+ args[2] + " for " + args[1] + " in " + args[3]);
				if (p != null)
					p.sendMessage(ChatColor.GRAY
							+ "[SH] You were granted permission " + args[2]
							+ " in " + args[3]);
				return true;
			}
		} else if (args.length > 0 && args[0].equalsIgnoreCase("whatshere"))
		{
			...
			{
				foundRegion = true;
				player.sendMessage(ChatColor.GRAY + "[SH] Found Region ID: "
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Found Super-Region named: " + ChatColor.GOLD
						+ sr.getName());
		...
		} else if (args.length > 1 && args[0].equalsIgnoreCase("info"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no region type called " + args[1]);
				return true;
			}
			if (rt != null)
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Info for region type " + ChatColor.GOLD
						+ args[1] + ":");
				player.sendMessage(ChatColor.GRAY + "Cost: " + ChatColor.GOLD
						+ rt.getMoneyRequirement() + ChatColor.GRAY
						+ ", Payout: " + ChatColor.GOLD + rt.getMoneyOutput()
						+ ChatColor.GRAY + ", Radius: " + ChatColor.GOLD
						+ (int) Math.sqrt(rt.getRadius()));

				String message = ChatColor.GRAY + "Description: "
						+ ChatColor.GOLD;
				int j = 0;
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] Info for super-region type " + ChatColor.GOLD
						+ args[1] + ":");
				player.sendMessage(ChatColor.GRAY + "Cost: " + ChatColor.GOLD
						+ srt.getMoneyRequirement() + ChatColor.GRAY
						+ ", Payout: " + ChatColor.GOLD + srt.getOutput());
				player.sendMessage(ChatColor.GRAY + "Power: " + ChatColor.GOLD
						+ srt.getMaxPower() + " (+" + srt.getDailyPower()
						+ "), " + ChatColor.GRAY + "Charter: " + ChatColor.GOLD
						+ srt.getCharter() + ChatColor.GRAY + ", Radius: "
						+ ChatColor.GOLD + (int) Math.sqrt(srt.getRadius()));
		...
		} else if (args.length > 1 && args[0].equalsIgnoreCase("addowner"))
		{
			...
			for (Region r : regionManager.getContainingBuildRegions(loc))
			{
								...
								"herostronghold.admin")))
				{
					if (r.isOwner(playername))
					{
						player.sendMessage(ChatColor.GRAY + "[SH] "
								+ playername
								+ " is already an owner of this region.");
						return true;
					}
					...
					player.sendMessage(ChatColor.GRAY + "[SH] "
							+ ChatColor.WHITE + "Added " + playername
							+ " as an owner.");
					if (aPlayer != null)
					{
						aPlayer.sendMessage(ChatColor.GRAY + "[SH] "
								+ ChatColor.WHITE + "You're now a co-owner of "
								+ player.getDisplayName() + "'s " + r.getType());
					}
					return true;
				} else
				{
							...
							.getContainingSuperRegions(loc))
					{
						...
						if (regionManager.getSuperRegionType(sr.getType())
								.hasEffect("control"))
						{
							takeover = true;
						}
					...
					if (takeover)
					{
						...
						if (aPlayer != null)
						{
							aPlayer.sendMessage(ChatColor.GRAY + "[SH] "
									+ ChatColor.WHITE
									+ "You're now a co-owner of "
									+ player.getDisplayName() + "'s "
									+ r.getType());
						}
						return true;
					} else
			...
			player.sendMessage(ChatColor.GRAY
					+ "[SH] You're not standing in a region.");
			return true;
			...
			for (Region r : regionManager.getContainingBuildRegions(loc))
			{
								...
								"herostronghold.admin")))
				{
					if (r.isMember(playername))
					{
						player.sendMessage(ChatColor.GRAY + "[SH] "
								+ playername
								+ " is already a member of this region.");
						return true;
					}
					if (r.isOwner(playername)
							&& !(playername.equals(player.getName()) && r
									.getOwners().get(0)
									.equals(player.getName())))
					{
						regionManager.setOwner(r, playername);
		...
		} else if (args.length > 2 && args[0].equals("addmemberid"))
		{
							...
							"herostronghold.admin")))
			{
				...
				if (r.isOwner(playername)
						&& !(playername.equals(player.getName()) && r
								.getOwners().get(0).equals(player.getName())))
				{
					regionManager.setOwner(r, playername);
				}
				regionManager.setMember(r, playername);
				player.sendMessage(ChatColor.GRAY + "[SH] " + ChatColor.WHITE
						+ "Added " + playername + " to the region.");
				return true;
			} else
			{
				player.sendMessage(ChatColor.GRAY
		...
		} else if (args.length > 1 && args[0].equalsIgnoreCase("whereis"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no region type " + args[1]);
				return true;
			}
			boolean found = false;
			for (Region r : regionManager.getSortedRegions())
			{
				...
				{
					player.sendMessage(ChatColor.GOLD + "[SH] " + args[1]
							+ " at " + ((int) r.getLocation().getX()) + ", "
								...
								"herostronghold.admin")))
				{
					...
					try
					{
						if (player.getLocation().distanceSquared(
								aPlayer.getLocation()) > regionManager
								.getRegionType(r.getType()).getRawRadius())
						{
							player.sendMessage(ChatColor.GRAY + "[SH] "
									+ playername + " must be close by also.");
							return true;
						}
					} catch (IllegalArgumentException iae)
					{
		...
		} else if (args.length > 1 && args[0].equalsIgnoreCase("remove"))
		{
			...
			for (Region r : regionManager.getContainingBuildRegions(loc))
			{
								...
								"herostronghold.admin")))
				{
					...
					{
						player.sendMessage(ChatColor.GRAY
								+ "[SH] You must use /hs setowner to change the original owner.");
						return true;
					}
					if (!r.isMember(playername) && !r.isOwner(playername))
					{
						player.sendMessage(ChatColor.GRAY + "[SH] "
								+ playername + " doesn't belong to this region");
						return true;
					}
					if (r.isMember(playername))
					{
					...
					player.sendMessage(ChatColor.GRAY + "[SH] "
							+ ChatColor.WHITE + "Removed " + playername
							+ " from the region.");
					return true;
				} else
				{
		...
		} else if (args.length == 1 && args[0].equalsIgnoreCase("destroy"))
		{
			...
			if (locationToDestroy != null)
			{
				...
				player.sendMessage(ChatColor.GRAY + "[SH] Region destroyed.");
			} else
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You're not standing in a region.");
		...
		} else if (args.length > 1 && args[0].equalsIgnoreCase("destroy"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] There is no region named " + args[1]);
				return true;
			}

			// Check if owner or admin of that region
			if ((perms == null || !perms.has(player, "herostronghold.admin"))
					&& (sr.getOwners().isEmpty() || !sr.getOwners().contains(
							player.getName())))
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You are not the owner of that region.");
				return true;
			}
		...
		{
			int j = 0;
			player.sendMessage(ChatColor.GRAY + "[SH] list of Region Types");
			String message = ChatColor.GOLD + "";
			boolean permNull = perms == null;
			boolean createAll = permNull
					|| perms.has(player, "herostronghold.create.all");
		...
		} else if (args.length > 2 && args[0].equalsIgnoreCase("rename"))
		{
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] That name is too long. Use 15 characters or less");
				return true;
			}
			...
			{
				player.sendMessage(ChatColor.GRAY
						+ "[SH] You don't have permission to rename that super-region.");
				return true;
			}

			double cost = configManager.getRenameCost();
			if (Stronghold.econ != null && cost > 0)
			{
				if (!Stronghold.econ.has(player.getName(), cost))
				{
					player.sendMessage(ChatColor.GRAY + "[SH] It costs "
							+ ChatColor.RED + cost + " to rename that.");
					return true;
				} else
				{
					Stronghold.econ.withdrawPlayer(player.getName(), cost);
				}
			}

			regionManager.destroySuperRegion(args[1], false);
			regionManager.addSuperRegion(args[2], sr.getLocation(),
					sr.getType(), sr.getOwners(), sr.getMembers(),
					sr.getPower(), sr.getBalance());
			player.sendMessage(ChatColor.GOLD + "[SH] " + args[1] + " is now "
					+ args[2]);
			return true;
		} else if (args.length > 0 && (args[0].equalsIgnoreCase("show")))
		{
			...
			return true;
				...
				&& (args[0].equalsIgnoreCase("stats") || args[0]
						.equalsIgnoreCase("who")))
		{
			if (args.length == 1)
			{
				...
				{
					player.sendMessage(ChatColor.GRAY + "[SH] ==:|"
							+ ChatColor.GOLD + r.getID() + " (" + r.getType()
							+ ") " + ChatColor.GRAY + "|:==");
			...
			if (sr != null)
			{
				// TODO make revenue include all owned regions within the super
				// region
				SuperRegionType srt = regionManager.getSuperRegionType(sr
						.getType());
				...
				if (srt.hasEffect("housing"))
				{
					...
					{
						housin += regionManager.getRegionType(r.getType())
								.getHousing();
					}
					housing = housin + "";
				}
				player.sendMessage(ChatColor.GRAY + "[SH] ==:|"
						+ ChatColor.GOLD + sr.getName() + " (" + sr.getType()
						+ ") " + ChatColor.GRAY + "|:==");
				player.sendMessage(ChatColor.GRAY
						+ "Population: "
						+ ChatColor.GOLD
						...
						+ ChatColor.GRAY
						+ " Bank: "
						+ (sr.getBalance() < srt.getOutput() ? ChatColor.RED
								: ChatColor.GOLD)
						+ sr.getBalance()
						+ ChatColor.GRAY
						+ " Power: "
						+ (sr.getPower() < srt.getDailyPower() ? ChatColor.RED
								: ChatColor.GOLD) + sr.getPower() + " (+"
				...
				player.sendMessage(ChatColor.GRAY
						...
						+ ChatColor.GRAY
						+ " Total Revenue: "
						+ (revenue < 0 ? ChatColor.RED : ChatColor.GOLD)
						+ revenue
						+ ChatColor.GRAY
						+ " Disabled: "
						+ (regionManager.hasAllRequiredRegions(sr) ? (ChatColor.GOLD + "false")
								: (ChatColor.RED + "true")));
						...
						|| sr.hasOwner(player.getName()))
				{
					player.sendMessage(ChatColor.GRAY + "Location: "
							+ ChatColor.GOLD + (int) sr.getLocation().getX()
							+ ", " + (int) sr.getLocation().getY() + ", "
							+ (int) sr.getLocation().getZ());
				}
				if (sr.getTaxes() != 0)
				{
					String message = ChatColor.GRAY + "Tax Revenue History: "
							+ ChatColor.GOLD;
			...
			if (p != null)
			{
				...
				player.sendMessage(ChatColor.GRAY + "[SH] "
						+ p.getDisplayName() + " is a member of:");
				String message = ChatColor.GOLD + "";
				int j = 0;
			...
			player.sendMessage(ChatColor.GRAY
					+ "[SH] Could not find player or super-region by that name");
			return true;
		} else if (args.length > 0 && effectCommands.contains(args[0]))
		{
			Bukkit.getServer().getPluginManager()
					.callEvent(new CommandEffectEvent(args, player));
			return true;
		} else
		{
			...
			{
				sender.sendMessage(ChatColor.GRAY + "[SH] by " + ChatColor.GOLD
						+ "Krglok" + ChatColor.GRAY
						+ ": <> = required, () = optional" + ChatColor.GOLD
						+ " Page 2");
				sender.sendMessage(ChatColor.GRAY + "/sh accept <name>");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh rename <name> <newname>");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh settaxes <amount> <name>");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh withdraw|deposit <amount> <name>");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh listperms <playername> <name>");
				sender.sendMessage(ChatColor.GRAY + "/sh listallperms");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh toggleperm <playername> <perm> <name>");
				sender.sendMessage(ChatColor.GRAY + "/sh destroy (name)");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh ch (channel) -- Use /hs ch for all chat");
				sender.sendMessage(ChatColor.GRAY
						+ "Google 'Stronghold bukkit' for more info | "
						+ ChatColor.GOLD + "Page 2/3");
			} else if (args.length > 0 && args[args.length - 1].equals("3"))
			{
				sender.sendMessage(ChatColor.GRAY + "[SH] by " + ChatColor.GOLD
						...
						+ " Page 3");
				// sender.sendMessage(ChatColor.GRAY +
				// "/hs war <mysuperregion> <enemysuperregion>");
				// sender.sendMessage(ChatColor.GRAY +
				// "/hs peace <mysuperregion> <enemysuperregion>");
				sender.sendMessage(ChatColor.GRAY
						+ "Google 'Stronghold bukkit' for more info | "
						+ ChatColor.GOLD + "Page 3/3");
			} else
			{
				sender.sendMessage(ChatColor.GRAY + "[SH] by " + ChatColor.GOLD
						+ "Krglok" + ChatColor.GRAY+"Ver. " +getDescription().getVersion() );
				sender.sendMessage(ChatColor.GRAY + ": () = optional"+ ChatColor.GOLD + " Page 1");
//				sender.sendMessage(ChatColor.GRAY + "Blazerod <RightClick> on ground for region info ");
				sender.sendMessage(ChatColor.GRAY + "/sh list");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh info <regiontype|superregiontype>");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh charter <superregiontype> <name>");
				sender.sendMessage(ChatColor.GRAY + "/sh charterstats <name>");
				sender.sendMessage(ChatColor.GRAY + "/sh signcharter <name>");
				sender.sendMessage(ChatColor.GRAY + "/sh cancelcharter <name>");
				sender.sendMessage(ChatColor.GRAY + "/sh create <regiontype>");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh create <superregiontype> (name)");
				sender.sendMessage(ChatColor.GRAY
						+ "/sh addowner|addmember|remove <playername> (name)");
				sender.sendMessage(ChatColor.GRAY + "/sh whatshere");
				sender.sendMessage(ChatColor.GRAY
						+ "Google 'Stronghold bukkit' for more info |"
						+ ChatColor.GOLD + " Page 1/3");
			}
	...
	public boolean setupEconomy()
	{
		...
		if (rsp != null)
		{
			...
			if (econ != null)
				System.out.println("[SH] Hooked into " + econ.getName());
		}
		return econ != null;
	}
	...
	private boolean setupPermissions()
	{
		...
		if (permissionProvider != null)
		{
			...
			if (perms != null)
				System.out.println("[SH] Hooked into " + perms.getName());
		}
		return (perms != null);
	}
