...
import multitallented.redcastlemedia.bukkit.stronghold.PermSet;
...
public class RegionManager
{
	...
	public RegionManager(Stronghold plugin, FileConfiguration config)
	{
		this.plugin = plugin;
		this.config = config;
...
//		load();
	...
	private void load()
	{
				...
				regionTypes
						.put(regionName,
								new RegionType(
										...
										(ArrayList<String>) rConfig
												.getStringList("friendly-classes"),
										(ArrayList<String>) rConfig
												.getStringList("enemy-classes"),
										(ArrayList<String>) rConfig
												.getStringList("effects"),
										(int) Math.pow(
												rConfig.getInt("radius"), 2),
										(int) Math.pow(rConfig.getInt(
												"build-radius",
												rConfig.getInt("radius", 2)), 2),
										processItemStackList(rConfig
												.getStringList("requirements"),
												currentRegionFile.getName()),
										rConfig.getStringList("super-regions"),
										processItemStackList(rConfig
												.getStringList("reagents"),
												currentRegionFile.getName()),
										processItemStackList(
												rConfig.getStringList("upkeep"),
												currentRegionFile.getName()),
										processItemStackList(
												rConfig.getStringList("output"),
												currentRegionFile.getName()),
										rConfig.getDouble("upkeep-chance"),
										rConfig.getDouble("money-requirement"),
										rConfig.getDouble("upkeep-money-output"),
										rConfig.getDouble("exp"), rConfig
												.getString("description"),
										rConfig.getInt("power-drain", 0),
										rConfig.getInt("housing", 0), rConfig
												.getStringList("biome")));
			} catch (Exception e)
			{
				plugin.warning("[Stronghold] failed to load "
						+ currentRegionFile.getName());
				e.printStackTrace();
			}
		}

		File suRegionFolder = new File(plugin.getDataFolder(),
				"SuperRegionConfig");
		if (!suRegionFolder.exists())
		{
			File sRegionFile = new File(plugin.getDataFolder(),
					"super-regions.yml");
		...
		for (File currentRegionFile : suRegionFolder.listFiles())
		{
			try
			{
				...
				String regionName = currentRegionFile.getName().replace(".yml",
						"");
				superRegionTypes.put(
						regionName,
						new SuperRegionType(regionName, rConfig
								.getStringList("effects"), (int) Math.pow(
								rConfig.getInt("radius"), 2),
								processRegionTypeMap(rConfig
										.getStringList("requirements")),
								rConfig.getDouble("money-requirement", 0),
								rConfig.getDouble("money-output-daily", 0),
								rConfig.getStringList("children"), rConfig
										.getInt("max-power", 100), rConfig
										.getInt("daily-power-increase", 10),
								rConfig.getInt("charter", 0), rConfig
										.getDouble("exp", 0), rConfig
										.getString("central-structure"),
								rConfig.getString("description"), rConfig
										.getInt("population", 0)));
			} catch (Exception e)
			{
				plugin.warning("[Stronghold] failed to load "
		...
		File playerFolder = new File(plugin.getDataFolder(), "data"); // Setup
																		// the
																		// Data
																		// Folder
																		// if it
		...
		for (File regionFile : playerFolder.listFiles())
		{
			try
			{
				...
				if (locationString != null)
				{
					...
					ArrayList<String> owners = (ArrayList<String>) dataConfig
							.getStringList("owners");
					ArrayList<String> members = (ArrayList<String>) dataConfig
					...
					if (location != null && type != null)
					{
						try
						{
							...
							liveRegions.put(
									location,
									new Region(Integer.parseInt(regionFile
											.getName().replace(".yml", "")),
											location, type, owners, members));

							sortedRegions.add(liveRegions.get(location));
							sortedBuildRegions.add(liveRegions.get(location));
							idRegions.put(liveRegions.get(location).getID(),
									liveRegions.get(location));
						} catch (NullPointerException npe)
						{
							System.out
									.println("[Stronghold] failed to load data from "
											+ regionFile.getName());
						}
					}
				}
			...
			{
				System.out.println("[Stronghold] failed to load data from "
						+ regionFile.getName());
		...
		if (sortedRegions.size() > 1)
		{
			Collections.sort(sortedRegions, new Comparator<Region>()
			{
				...
				{
					return (int) (-1 * (o1.getLocation().getX()
							+ getRegionType(o1.getType()).getRawRadius() - (o2
							.getLocation().getX() + getRegionType(o2.getType())
							.getRawRadius())));
				}
			});
		}
		if (sortedBuildRegions.size() > 1)
		{
			Collections.sort(sortedBuildRegions, new Comparator<Region>()
			{
				...
				{
					return (int) (-1 * (o1.getLocation().getX()
							+ getRegionType(o1.getType()).getRawBuildRadius() - (o2
							.getLocation().getX() + getRegionType(o2.getType())
							.getRawBuildRadius())));
				}
			});
		...
		// Load super regions
		File sRegionFolder = new File(plugin.getDataFolder(), "superregions"); // Setup
																				// the
																				// Data
																				// Folder
																				// if
		...
		for (File sRegionFile : sRegionFolder.listFiles())
		{
			try
			{
				...
				if (locationString != null)
				{
					...
					ArrayList<String> owners = (ArrayList<String>) sRegionDataConfig
							.getStringList("owners");
					ConfigurationSection configMembers = sRegionDataConfig
					...
					double balance = sRegionDataConfig
							.getDouble("balance", 0.0);
					List<Double> taxRevenue1 = sRegionDataConfig
							.getDoubleList("tax-revenue");
					LinkedList<Double> taxRevenue = new LinkedList<Double>();
					int maxPower = sRegionDataConfig.getInt("max-power", this
							.getSuperRegionType(type).getMaxPower());
					if (taxRevenue1 != null)
					{
					...
					if (location != null && type != null)
					{
						liveSuperRegions.put(name, new SuperRegion(name,
								location, type, owners, members, power, taxes,
								balance, taxRevenue, maxPower));
			...
			{
				System.out
						.println("[Stronghold] failed to load superregions from "
								+ sRegionFile.getName());
				e.printStackTrace();
			}
		}
			...
			if (sortedSuperRegions.size() > 1)
			{
				Collections.sort(sortedSuperRegions,
						new Comparator<SuperRegion>()
						{
							...
							{
								return (int) (-1 * (o1.getLocation().getX()
										+ getSuperRegionType(o1.getType())
												.getRawRadius() - (o2
										.getLocation().getX() + getSuperRegionType(
										o2.getType()).getRawRadius())));
							}
						});
			}
		}
		...
		} catch (Exception ioe)
		{
			...
			log.warning("[Stronghold] failed to load war.yml");
		}
	...
	private Map<String, Integer> processRegionTypeMap(List<String> input)
	{
		...
		for (String s : input)
		{
			...
			if (currentRegionType != null)
				tempMap.put(currentRegionType.getName(),
			...
			String filename)
	{
		...
		for (String current : input)
		{
			...
			if (Material.getMaterial(params[0]) != null)
			{
				...
				if (params.length < 3)
				{
					is = new ItemStack(Material.getMaterial(params[0]),
							Integer.parseInt(params[1]));
				} else
				{
					is = new ItemStack(Material.getMaterial(params[0]),
							Integer.parseInt(params[1]),
							Short.parseShort(params[2]));
				}
				returnList.add(is);
			} else
			{
				plugin.warning("[Stronghold] could not find item " + params[0]
						+ " in " + filename);
			}
		}
		return returnList;
	...
	public void addRegion(Location loc, String type, ArrayList<String> owners)
	{
		...
		try
		{
			...
			dataConfig = new YamlConfiguration();
			System.out.println("[Stronghold] saving new region to " + i
					+ ".yml");
			// dataConfig.load(dataFile);

			dataConfig.set(
					"location",
					loc.getWorld().getName() + ":" + loc.getX() + ":"
			...
			liveRegions.put(loc, new Region(i, loc, type, owners,
			...
			if (sortedBuildRegions.size() > 1)
			{
				Collections.sort(sortedBuildRegions, new Comparator<Region>()
				{
					...
					{
						return (int) (-1 * (o1.getLocation().getX()
								+ getRegionType(o1.getType())
										.getRawBuildRadius() - (o2
								.getLocation().getX() + getRegionType(
								o2.getType()).getRawBuildRadius())));
					}
				});
			}
			...
			plugin.getServer().getPluginManager()
					.callEvent(new RegionCreatedEvent(liveRegions.get(loc)));
		} catch (Exception ioe)
		{
			System.out
					.println("[Stronghold] unable to write new region to file "
							+ i + ".yml");
			ioe.printStackTrace();
		}
	}
	...
	public boolean addSuperRegion(String name, Location loc, String type,
			List<String> owners, Map<String, List<String>> members, int power,
			double balance)
	{
		File superRegionFolder = new File(plugin.getDataFolder()
				+ "/superregions");
		...
		try
		{
			dataFile.createNewFile();
			dataConfig = new YamlConfiguration();
			System.out.println("[Stronghold] saving new superregion to " + name
					+ ".yml");
			...
			liveSuperRegions.put(name, new SuperRegion(name, loc, type, owners,
					members, power, 0.0, 0.0, new LinkedList<Double>(),
					maxPower));
				...
				if (sortedSuperRegions.size() > 1)
				{
					Collections.sort(sortedSuperRegions,
							new Comparator<SuperRegion>()
							{
								...
								@Override
								public int compare(SuperRegion o1,
										SuperRegion o2)
								{
									return (int) (-1 * (o1.getLocation().getX()
											+ getSuperRegionType(o1.getType())
			...
			plugin.getServer().getPluginManager()
					.callEvent(new SuperRegionCreatedEvent(name));
			return true;
		} catch (Exception ioe)
		{
			System.out
					.println("[Stronghold] unable to write new superregion to file "
							+ name + ".yml");
			ioe.printStackTrace();
			return false;
		}
	...
	{
		// //////////////////////////////////////////
		// Note: this method does not remove the region due to Concurrent
		...
		{
			System.out.println("[Stronghold] Successfully destroyed region "
					+ currentRegion.getID() + ".yml");
		}
			...
			public void run()
			{
				for (Player p : plugin.getServer().getOnlinePlayers())
				{
					try
					{
						...
						{
							p.sendMessage(ChatColor.GRAY + "[Stronghold] "
									+ ChatColor.WHITE + regionTypeName
									+ " was disabled!");
		...
		plugin.getServer().getPluginManager()
				.callEvent(new RegionDestroyedEvent(currentRegion));
		if (configManager.getExplode())
		{
	...
	public void destroySuperRegion(String name, boolean sendMessage)
	{
		...
		File dataFile = new File(plugin.getDataFolder() + "/superregions", name
				+ ".yml");
		if (!dataFile.exists())
		{
			System.out
					.println("[Herostronghold] Unable to destroy non-existent superregion "
							+ name + ".yml");
			return;
		}
		if (!dataFile.delete())
		...
		{
			System.out
					.println("[Stronghold] Successfully destroyed superregion "
							+ name + ".yml");
		}
			...
			new Runnable()
			{
				...
				public void run()
				{
					plugin.getServer().broadcastMessage(
							ChatColor.GRAY + "[Stronghold] " + ChatColor.RED
									+ regionName + " was destroyed!");
				}
			}.run();
		}
		removeWars(name);
		...
		Bukkit.getPluginManager().callEvent(
				new SuperRegionDestroyedEvent(currentRegion));
	}
	...
	public void checkIfDestroyedSuperRegion(Location loc)
	{
		...
		{
			if (or.getType().equals(
					getSuperRegionType(sr.getType()).getCentralStructure()))
			{
	...
	public ArrayList<Region> getContainedRegions(SuperRegion sr)
	{
		...
		for (Region r : getSortedBuildRegions())
		{
			...
			if (l.getX() - radius < x && l.getY() + radius > y
					&& l.getY() - radius < y && l.getZ() + radius > z
					&& l.getZ() - radius < z
	...
	public boolean hasAllRequiredRegions(SuperRegion sr)
	{
		...
		for (Region r : getSortedRegions())
		{
					...
					&& l.getWorld().equals(loc.getWorld()))
			{
				...
				} else if (reqs.containsKey(getRegionType(r.getType())
						.getName()))
				{
	...
	{
		ConfigManager cm = Stronghold.getConfigManager();
		if (!cm.getUsePower())
		{
			return;
		}
					...
					plugin.getServer()
							.broadcastMessage(
									ChatColor.RED
											+ "[Stronghold] "
											+ st
											+ " reached 25 power! Destruction is near!");
				}
			}.run();
					...
					plugin.getServer()
							.broadcastMessage(
									ChatColor.RED
											+ "[Stronghold] "
											+ st
											+ " reached 10 power! Destruction is at hand!");
				}
			}.run();
	...
	public synchronized void setPower(SuperRegion sr, int newPower)
	{
		...
		File superRegionFile = new File(plugin.getDataFolder()
				+ "/superregions", sr.getName() + ".yml");
		...
		{
			plugin.warning("Failed to load " + sr.getName()
					+ ".yml to save new Power");
			return;
		}
	...
	public void setMember(Region r, String name)
	{
		...
		File regionFile = new File(plugin.getDataFolder() + "/data", r.getID()
				+ ".yml");
		if (!regionFile.exists())
		{
		...
		{
			plugin.warning("Failed to load " + r.getID()
					+ ".yml to save member");
			return;
		}
	...
	/**
	 * Adds (or subtracts if negative) the balance from the super-region. It
	 * saves that data to sr.getName() + ".yml". If the new balance would be
	 * less than 0, it takes the remainder and subtracts that from the owner's
	 * balance.
	 */
	public synchronized double addBalance(SuperRegion sr, double balance)
	{
		...
		if (!superRegionFile.exists())
		{
			plugin.warning("Failed to save " + sr.getName()
		...
		{
			plugin.warning("Failed to load " + sr.getName()
					+ ".yml to save new bank balance");
			return 0;
		}
		double newBalance = balance + sr.getBalance();
		if (balance < 0)
		{
			if (newBalance < 0 && Stronghold.econ != null)
			{
				String ownerName = sr.getOwners().get(0);
				double ownerBalance = Stronghold.econ.bankBalance(ownerName).balance;
				if (newBalance + ownerBalance <= 0 && ownerBalance != 0)
				{
					Stronghold.econ.withdrawPlayer(ownerName, ownerBalance);
					...
					if (p != null && p.isOnline())
					{
						p.sendMessage(ChatColor.RED
								+ "[Stronghold] "
								+ sr.getName()
								+ " and you are out of money. Do something fast!");
					}
				} else
				{
					Stronghold.econ.withdrawPlayer(ownerName, -newBalance);
				}
	...
	public ArrayList<Region> getContainingRegions(Location loc)
	{
		...
		double y = loc.getY();
		y = y < 0 ? 0 : y;
		y = y > loc.getWorld().getMaxHeight() ? loc.getWorld().getMaxHeight()
				: y;
		double z = loc.getZ();
		for (Region r : getSortedRegions())
		{
	...
	public ArrayList<Region> getContainingBuildRegions(Location loc)
	{
		...
		for (Region r : sortedBuildRegions)
		{
			try
			{
				...
				radius = radius < 1 ? getRegionType(r.getType()).getRawRadius()
						: radius;
				Location l = r.getLocation();
				if (l.getX() + radius < x)
				{
	...
	@Deprecated
	public boolean shouldTakeAction(Location loc, Player player, int modifier,
			String effectName, boolean useReagents)
	{
		...
		for (Region r : this.getContainingRegions(loc, modifier))
		{
			...
			if (!nullPlayer)
			{
				if ((r.isMember(player.getName()) || r
						.isOwner(player.getName())))
				{
					member = true;
			...
			if (!useReagents
					&& (nullPlayer || !member)
					&& effect.regionHasEffect(getRegionType(r.getType())
							.getEffects(), effectName) != 0)
			{
				return true;
			}
			if (useReagents
		...
		for (SuperRegion sr : this.getContainingSuperRegions(loc))
		{
			...
			if (!nullPlayer)
			{
				member = (sr.hasOwner(player.getName()) || sr.hasMember(player
						.getName()));
			}
			boolean reqs = hasAllRequiredRegions(sr);
			boolean hasEffect = getSuperRegionType(sr.getType()).hasEffect(
					effectName);
			boolean hasPower = sr.getPower() > 0;
			boolean hasMoney = sr.getBalance() > 0;
			if (useReagents && (nullPlayer || !member) && hasEffect && reqs
					&& hasPower && hasMoney)
			{
				return true;
			}
	...
	public boolean shouldTakeAction(Location loc, Player player,
			ArrayList<RegionCondition> conditions)
	{
		Effect effect = new Effect(plugin);
		...
		{
			if (!conditionJA.containsKey(rc.MODIFIER)
					|| conditionJA.get(rc.MODIFIER) == null)
			{
		...
		for (Integer i : conditionJA.keySet())
		{
			for (Region r : this.getContainingRegions(loc, i))
			{
				...
				if (!nullPlayer)
				{
					if ((r.isMember(player.getName()) || r.isOwner(player
							.getName())))
					{
						member = true;
					} else if (r.isMember("all"))
					...
					} else
					{
						for (String s : r.getMembers())
						{
							if (s.contains("sr:"))
							{
								SuperRegion sr = getSuperRegion(s.replace(
										"sr:", ""));
								if (sr != null
										&& (sr.hasMember(player.getName()) || sr
												.hasOwner(player.getName())))
				...
				for (RegionCondition rc : conditionJA.get(i))
				{
					...
					if (!useReagents
							&& (nullPlayer || !member)
							&& effect.regionHasEffect(
									getRegionType(r.getType()).getEffects(),
									effectName) != 0)
					{
						return true;
					}
			...
			for (SuperRegion sr : this.getContainingSuperRegions(loc))
			{
				...
				if (!nullPlayer)
				{
					member = (sr.hasOwner(player.getName()) || sr
							.hasMember(player.getName()));
				}
				boolean reqs = hasAllRequiredRegions(sr);
				boolean hasPower = sr.getPower() > 0;
				boolean hasMoney = sr.getBalance() > 0;
				...
				for (RegionCondition rc : conditionJA.get(i))
				{
					String effectName = rc.NAME;
					boolean useReagents = rc.USE_REAGENTS;
					boolean hasEffect = getSuperRegionType(sr.getType())
							.hasEffect(effectName);
					if (useReagents && (nullPlayer || !member) && hasEffect
							&& reqs && hasPower && hasMoney)
					{
						return true;
					}
					if (!useReagents && (nullPlayer || !member) && hasEffect)
	...
	/*
	 * public boolean shouldTakeActionDebug(Location loc, Player player,
	 * ArrayList<RegionCondition> conditions) { Logger log =
	 * Logger.getLogger("Minecraft"); Effect effect = new Effect(plugin);
	 * HashMap<Integer, ArrayList<RegionCondition>> conditionJA = new
	 * HashMap<Integer, ArrayList<RegionCondition>>(); for (RegionCondition rc :
	 * conditions) { if (conditionJA.containsKey(rc.MODIFIER) &&
	 * conditionJA.get(rc.MODIFIER) != null &&
	 * !conditionJA.get(rc.MODIFIER).isEmpty()) { conditionJA.put(rc.MODIFIER,
	 * new ArrayList<RegionCondition>()); conditionJA.get(rc.MODIFIER).add(rc);
	 * } else { conditionJA.put(rc.MODIFIER, new ArrayList<RegionCondition>());
	 * conditionJA.get(rc.MODIFIER).add(rc); } } for (Integer i :
	 * conditionJA.keySet()) { for (Region r : this.getContainingRegions(loc,
	 * i)) { boolean nullPlayer = player == null; boolean member = false; if
	 * (!nullPlayer) { if ((r.isMember(player.getName()) ||
	 * r.isOwner(player.getName()))) { member = true; } else if
	 * (r.isMember("all")) { member = true; } else { for (String s :
	 * r.getMembers()) { if (s.contains("sr:")) { SuperRegion sr =
	 * getSuperRegion(s.replace("sr:", "")); if (sr != null &&
	 * (sr.hasMember(player.getName()) || sr.hasOwner(player.getName()))) {
	 * member = true; } } } } }
	 * 
	 * for (RegionCondition rc : conditionJA.get(i)) { boolean useReagents =
	 * rc.USE_REAGENTS; String effectName = rc.NAME; String message = rc.NAME +
	 * " : " + useReagents + " : " + (nullPlayer || !member) + " : " +
	 * (effect.regionHasEffect(getRegionType(r.getType()).getEffects(),
	 * effectName) != 0) + " : " + effect.hasReagents(r.getLocation());
	 * log.info(message); if (!useReagents && (nullPlayer || !member) &&
	 * effect.regionHasEffect(getRegionType(r.getType()).getEffects(),
	 * effectName) != 0) { log.info("denied"); return true; } if (useReagents &&
	 * (nullPlayer || !member) &&
	 * effect.regionHasEffect(getRegionType(r.getType()).getEffects(),
	 * effectName) != 0 && effect.hasReagents(r.getLocation())) {
	 * log.info("denied"); return true; } } } for (SuperRegion sr :
	 * this.getContainingSuperRegions(loc)) { boolean nullPlayer = player ==
	 * null; boolean member = false; if (!nullPlayer) { member =
	 * (sr.hasOwner(player.getName()) || sr.hasMember(player.getName())); }
	 * boolean reqs = hasAllRequiredRegions(sr); boolean hasPower =
	 * sr.getPower() > 0; boolean hasMoney = sr.getBalance() > 0;
	 * 
	 * 
	 * for (RegionCondition rc : conditionJA.get(i)) { String effectName =
	 * rc.NAME; boolean useReagents = rc.USE_REAGENTS; boolean hasEffect =
	 * getSuperRegionType(sr.getType()).hasEffect(effectName); String message =
	 * rc.NAME + ": " + useReagents + " : " + (nullPlayer || !member) + " : " +
	 * hasEffect + " : " + reqs + " : " + hasPower + " : " + hasMoney;
	 * log.info(message); if (useReagents && (nullPlayer || !member) &&
	 * hasEffect && reqs && hasPower && hasMoney) { log.info("denied"); return
	 * true; } if (!useReagents && (nullPlayer || !member) && hasEffect) {
	 * log.info("denied"); return true; } } } } log.info("not denied"); return
	 * false; }
	 */
	...
	public void setWar(SuperRegion sr1, SuperRegion sr2)
	{
		...
		try
		{
			if (!warFile.exists())
			{
				System.out.println("[Stronghold] failed to load war.yml");
				return;
			}
		} catch (Exception e)
		{
		...
		if (hasWar(sr1, sr2))
		{
			try
			{
				...
				{
					List<String> tempList = warConfig.getStringList(sr1
							.getName());
				...
				{
					List<String> tempList = warConfig.getStringList(sr2
							.getName());
			...
			{
				System.out
						.println("[Stronghold] failed to remove war from war.yml");
				return;
			}
		} else
		{
			...
			{
				System.out
						.println("[Stronghold] failed to save new war to war.yml");
				return;
			}
		}
	}
	...
	public void removeWars(String name)
	{
		...
		{
			System.out.println("[Stronghold] Failed to save war.yml");
		}
	}
	...
	public boolean isAtMaxRegions(Player p, RegionType rt)
	{
		...
		for (String s : sets)
		{
			if (permSets.containsKey(s)
					&& permSets.get(s).getPriority() > highestPriority)
			{
				ps = permSets.get(s);
	...
	public ArrayList<String> getPermSets(Player p)
	{
		...
		{
			if (Stronghold.perms.has(p, "herostronghold.group." + s))
			{
				sets.add(s);
			}
		}
	...
	public boolean canBuildHere(Player p, Location l)
	{
		...
		for (Region r : getContainingRegions(l))
		{
			...
			{
				continue;
			} else if ((effect.regionHasEffect(r, "denyblockbuild") != 0 && effect
		...
		for (SuperRegion sr : getContainingSuperRegions(l))
		{
			...
			{
				continue;
			} else if (srt.hasEffect("denyblockbuild")
					|| srt.hasEffect("denyblockbuildnoreagent"))
			{
				return false;
			}
		}
	...
	public boolean canBreakHere(Location l, Player p)
	{
		...
		for (Region r : getContainingRegions(l))
		{
			...
			{
				continue;
			} else if ((effect.regionHasEffect(r, "denyblockbreak") != 0 && effect
					.hasReagents(r.getLocation()))
		...
		for (SuperRegion sr : getContainingSuperRegions(l))
		{
			...
			{
				continue;
			} else if (srt.hasEffect("denyblockbreak")
					|| srt.hasEffect("denyblockbreaknoreagent"))
			{
				return false;
			}
		}
